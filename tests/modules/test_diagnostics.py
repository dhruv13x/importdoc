...
def test_import_diagnostic_init():
    ...
    assert diagnostic.use_emojis is True
    assert diagnostic.log_file is None
    assert diagnostic.timeout == 0
    assert diagnostic.dry_run is False
    assert diagnostic.unload is False
    assert diagnostic.json_output is False
    assert diagnostic.parallel == 0
    assert diagnostic.max_depth is None
    assert diagnostic.dev_mode is False
    assert diagnostic.dev_trace is False
    ...
    assert diagnostic.safe_skip_imports is True
    assert diagnostic.max_scan_results == 200


@patch("importdoc.modules.diagnostics.import_module_worker")
def test_import_discovered_modules_success(mock_import_module_worker):
    ...
    assert "os" in diagnostic.imported_modules
    assert "sys" in diagnostic.imported_modules
    assert len(diagnostic.failed_modules) == 0
...
def test_import_discovered_modules_failure(mock_import_module_worker):
    ...
    assert "non_existent_module" not in diagnostic.imported_modules
    assert len(diagnostic.failed_modules) == 1
    assert diagnostic.failed_modules[0][0] == "non_existent_module"
    assert "Test error" in diagnostic.failed_modules[0][1]
...
def test_handle_error_no_module_named():
    ...
    assert "No module named 'my_module'" in diagnostic.failed_modules[0][1]
...
def test_handle_error_cannot_import_name():
    ...
    assert (
        ...
        in diagnostic.failed_modules[0][1]
    )
...
@patch("importdoc.modules.diagnostics.ImportDiagnostic._discover_all_modules", return_value=None)
@patch("importdoc.modules.diagnostics.tempfile.NamedTemporaryFile")
@patch("importdoc.modules.diagnostics.os.replace")
@patch("importdoc.modules.diagnostics.json.dump")
...
def test_discover_all_modules(mock_find_spec):
    ...
    # This side effect is key: it returns a spec for the root package,
    # but not for the submodules, simulating how find_spec works for file-based modules.
    def find_spec_side_effect(name, *args, **kwargs):
        if name == "my_package":
            return main_spec
        # It will also be called for "my_package.my_module" if it were a directory
        return None

    mock_find_spec.side_effect = find_spec_side_effect

    with patch("importdoc.modules.diagnostics.Path") as mock_path_class:
        # Mock the instance of the Path object
        mock_path_instance = mock_path_class.return_value
        mock_path_instance.exists.return_value = True

        # Create mocks for the directory entries
        mock_file = MagicMock()
    ...
    assert "my_package.my_module" in diagnostic.discovered_modules
...
def test_diagnose_path_issue(mock_find_module_file_path):
    ...
    with patch.object(diagnostic, "_log") as mock_log:
        ...
        mock_log.assert_any_call("No file found matching module.", level="INFO")
        mock_log.assert_any_call("Current sys.path:", level="INFO")
...
def test_install_uninstall_import_tracer():
    ...
    diagnostic._uninstall_import_tracer()
    assert __builtins__["__import__"] == original_import
...
def test_export_graph(mock_mkdir, mock_os_replace, mock_tempfile):
    ...
    diagnostic._export_graph()

    mock_mkdir.assert_called_with(parents=True, exist_ok=True)
    mock_file.write.assert_any_call("digraph imports {\n")
    mock_file.write.assert_any_call('  "a" -> "b" [color=green, penwidth=2];\n')
    mock_file.write.assert_any_call('  "b" -> "c" [color=green, penwidth=2];\n')
...
@patch("importdoc.modules.diagnostics.ImportDiagnostic._validate_package", return_value=True)
@patch("importdoc.modules.diagnostics.ImportDiagnostic._discover_all_modules")
@patch("importdoc.modules.diagnostics.ImportDiagnostic._import_discovered_modules")
...
def test_print_summary_with_telemetry(mock_get_summary):
    ...
    with patch.object(diagnostic, "_log") as mock_log:
        ...
        mock_log.assert_any_call("  Total events: 1", level="INFO")
...
@patch("importdoc.modules.diagnostics.logging.getLogger")
@patch("importdoc.modules.diagnostics.RotatingFileHandler")
def test_setup_logger_with_file(mock_file_handler, mock_get_logger):
    # Reset the initialized flag from any previous test to ensure the handler is set up
    logger_instance = mock_get_logger.return_value
    if hasattr(logger_instance, "_initialized_by_import_diag"):
...
def test_analyze_error_context_no_module_named_local_submodule(mock_find_spec):
    def find_spec_side_effect(name):
        if name == "my_package":
            return True
        elif name == "my_package.sub_module":
            return None
        return True
...
@patch("importdoc.modules.diagnostics.is_standard_lib")
@patch("importdoc.modules.diagnostics.suggest_pip_names", return_value=["my_package"])
def test_analyze_error_context_no_module_named_external_dependency(
    mock_suggest_pip_names, mock_is_standard_lib
):
    def is_standard_lib_side_effect(name):
        if name == "my_package":
            return False
        return True
